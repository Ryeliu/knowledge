...
│   └── generate_ppt()        ← PPT outline + slide images + PDF (OpenRouter/Gemini)
├── Skills
│   ├── /meeting       ← Audio → structured meeting notes → auto-ingest
│   ├── /archive       ← inbox/ files → identify company → archive
│   └── /ppt           ← Knowledge base context → generate_ppt() → output PDF
...
- **PPT Generation** — `/ppt` skill + `generate_ppt()` MCP tool: pulls context from knowledge base → plans outline via Gemini → generates slide images → outputs PDF to `output/`. No separate backend needed.
...
- **PPT Generation** (MCP server): `openai` (OpenRouter/Gemini API), `Pillow`
...
1. Searches the knowledge base for company context via `search_knowledge()`
2. Calls `generate_ppt()` MCP tool (Gemini for outline + slide images)
3. Outputs slides as images and a combined PDF in `output/`
